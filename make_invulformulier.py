...
SPELREGELS = [
    ...
    "Na je wedstrijdscores vul je per groep de groepswinnaar en runner-up in, én de 8 beste nummers 3. Dit zijn vrije voorspellingen — ze mogen afwijken van wat je scores logisch opleveren.",
...
def build_group_matches():
    ...
    for g, teams in GROUPS.items():
        for n, (a, b) in enumerate(GROUP_PAIRS[g], 1):
...
def build_sheet(ws, ls):  # ls = naam van het Landen-sheet
    ...
    for i in range(0, 12, 2):
        ...
        # Groepswinnaar + Runner-up: vrij invullen met dropdown per groep
        group_keys = list(GROUPS.keys())
        gl_col = get_column_letter(4 + group_keys.index(gl))
        gr_col = get_column_letter(4 + group_keys.index(gr))
        grp_l_f = f"={ls}!${gl_col}$1:${gl_col}$4"
        grp_r_f = f"={ls}!${gr_col}$1:${gr_col}$4"
        for adv_lbl in ["Groepswinnaar:", "Runner-up:"]:
            rh(ws, row, 17)
            lbl(ws, row, C_CLR1, C_AWAY, adv_lbl)
            inp(ws, row, C_PRED)
            dropdown(ws, row, C_PRED, grp_l_f)
            grey(ws, row, C_RES)
            grey(ws, row, C_PT)
            lbl(ws, row, C_CLR3, C_AWAY2, adv_lbl)
            inp(ws, row, C_PRED2)
            dropdown(ws, row, C_PRED2, grp_r_f)
            grey(ws, row, C_RES2)
            grey(ws, row, C_PT2)
            row += 1
    ...
    # ── Beste 8 nummers 3 ─────────────────────────────────────────────────────
    row = sec_hdr(ws, row, "▶  BESTE 8 NUMMERS 3  ·  vul in welke 8 nummers 3 jij verwacht dat doorgaan")
    for batch in range(2):
        rh(ws, row, 18)
        for i in range(4):
            nr = batch * 4 + i + 1
            start = i * 4 + 1          # 1, 5, 9, 13
            c = mc(ws, row, start, start + 1)
            c.value, c.font, c.alignment = f"#{nr}:", fnt(bold=True, color="2A5298"), AL_R
            inp(ws, row, start + 2)
            dropdown(ws, row, start + 2, all_f)
        row += 1
    row = empty(ws, row, 8)
...
def build_landen_sheet(ws):
    ...
    for col_idx, teams in enumerate(GROUPS.values(), 4):             # D-O: teams per groep
        for i, t in enumerate(teams, 1):
            ws.cell(i, col_idx).value = t
